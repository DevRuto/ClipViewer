...
public class VideoJobQueueTests
{
    ...
    [Fact]
    public async Task EnqueueAsync_SetsOutputFilePath()
    {
        // Arrange
        var channel = Channel.CreateUnbounded<VideoConversionJob>();
        var mockLogger = new Mock<ILogger<VideoJobQueue>>();
        ...
        Assert.StartsWith("output", job.OutputFilePath);
        Assert.Contains(Path.DirectorySeparatorChar, job.OutputFilePath);
        Assert.EndsWith(".mp4", job.OutputFilePath, StringComparison.OrdinalIgnoreCase);
        Assert.Equal(job.OutputFilePath, Path.Combine("output", result));
    }

    [Fact]
    public async Task EnqueueAsync_WithDifferentExtensions_PreservesExtension()
    {
        // Arrange
        var channel = Channel.CreateUnbounded<VideoConversionJob>();
        var mockLogger = new Mock<ILogger<VideoJobQueue>>();
        var queue = new VideoJobQueue(channel, mockLogger.Object);
        var job1 = new VideoConversionJob("input.mov", "output", Guid.NewGuid());
        var job2 = new VideoConversionJob("input.avi", "output", Guid.NewGuid());
        ...
        // Assert
        Assert.EndsWith(".mov", job1.OutputFilePath);
        Assert.EndsWith(".avi", job2.OutputFilePath);
    }
}